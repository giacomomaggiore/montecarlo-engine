...
- [ ] **Phase 1 — Bootstrap engine + portfolio accounting, headless.** Build the following in `core/` only, with no dataset fetch, binary parsing, worker, chart, or real input UI. Keep scenario generation separate from portfolio accounting so later engines reuse the same accounting loop.
  ...
  - [ ] **Phase 1.2 — Versioned deterministic randomness.** Add `core/math/random.ts` with a local `xoshiro128**` generator. Expand one 32-bit seed into four state words with one documented, versioned rule; reject the all-zero state; expose unsigned-32-bit, $[0,1)$ uniform, and unbiased bounded-integer draws; never call `Math.random()`. Test fixed reference vectors, same-seed reproducibility, different-seed divergence, output bounds, and row-index bounds. Keep this module independent of simulation and React code.
  - [ ] **Phase 1.3 — Historical joint bootstrap engine.** Add `core/simulation/historicalBootstrap.ts` implementing the `SimulationEngine` contract. For each period, draw one aligned historical row with replacement and emit every selected asset return, inflation increment, risk-free rate, and the original row index from that same row. Never sample asset columns independently and never perform portfolio accounting inside the engine. Test exact seeded row sequences, whole-row identity, source-row metadata, and approximately uniform row frequencies using a fixed sample size and tolerance chosen before running the test.
  - [ ] **Phase 1.4 — Minimal unleveraged portfolio accounting.** Add `core/portfolio/cashFlows.ts` and the smallest pure accounting function needed by the runner. Allocate the initial investment at target weights; each period apply simple returns to current holdings, calculate the pre-contribution equity, then add the scheduled contribution and invest only that contribution at target proportions. Implement lump sum, fixed end-of-period DCA, and contribution-only value averaging with $A_t=A_0+t\Delta$ and $C_t=\max(0,A_t-E_t^-)$; never rebalance existing holdings or withdraw excess value. Record holdings, actual external contribution, equity, and contribution-neutral return. Test one- and two-asset hand calculations, return-before-contribution timing, weight drift, no-withdrawal value averaging, and value conservation. Taxes, costs, debt, benchmark, rebalancing, and leverage remain absent.
  - [ ] **Phase 1.5 — Headless runner and quantiles.** Add `core/math/quantiles.ts` and `core/simulation/runSimulation.ts`. For each path, request one scenario per period from the engine and pass it unchanged to the accounting function. Enforce the 50,000-path and $N\times T\le10{,}000{,}000$ limits. Retain terminal wealth for every path, values from period zero onward needed for aggregate p10/p25/p50/p75/p90 bands, and deterministic full details for at most 50 paths. Use one versioned linear-interpolation rule with $h=(n-1)q$ for every quantile. Return the full configuration, dataset version and checksum, seed, PRNG/model/quantile versions, terminal outcomes, quantiles, retained paths, and explicit run/path failures; do not return or silently replace non-finite results.
  - [ ] **Phase 1.6 — Proof and completion gate.** Colocate Vitest tests with every `core/` module. Cover one-asset compounding, all three cash-flow modes, aligned-row bootstrap behavior, seeded reproducibility, quantile interpolation, invalid inputs, and accounting invariants. Validate one small seeded simulation period by period against an independent spreadsheet or Python calculation, including sampled row, holdings, contribution, neutral return, and equity rather than terminal wealth alone. Run formatting, linting, strict TypeScript checking, unit/statistical tests, and a production build; document PRNG/model/quantile versions and results in `LOG.MD`. Phase 1 is complete only when these checks pass without using React or visual chart inspection.
